...
const URGENCY_STYLE = {
  ...
  optional:     { badge: 'bg-slate-100 text-slate-600',  label: 'Optional' },
};
...
function TestCard({ test, index }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow p-5 print:shadow-none print:border print:break-inside-avoid">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-800 leading-tight">{test.test_name}</h3>
            {test.abbreviation && (
              <span className="text-xs font-mono text-slate-400">{test.abbreviation}</span>
            )}
          </div>
        </div>
      ...
      {test.normal_range && (
        <div className="flex items-center gap-2 text-xs text-slate-500 mb-2">
          ...
          <span className="font-mono bg-slate-50 px-2 py-0.5 rounded">{test.normal_range}</span>
        </div>
      )}
...
export default function Tests() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 print:space-y-4">
      {/* Breadcrumb */}
      <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs text-slate-400 print:hidden">
        ...
        <span>›</span>
        <span className="text-slate-600 font-medium">Blood Tests</span>
      </nav>

      {/* Header */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow p-6 print:shadow-none">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h1 className="text-xl font-bold text-slate-800">Recommended Blood Tests</h1>
            {disease && (
              <p className="text-sm text-slate-500 mt-0.5">
                For: <span className="font-medium text-blue-600">{disease.disease}</span>
                {disease.icd_code && (
                  <span className="text-xs text-slate-400 ml-2 font-mono">({disease.icd_code})</span>
                )}
              </p>
            )}
          </div>
          {!loading && tests.length > 0 && (
            <button
              ...
              className="print:hidden px-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 flex items-center gap-2"
            >
              🖨 Print / Save PDF
            </button>
          )}
      ...
      {/* Loading states */}
      {(loadingDb || loading) && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow p-10 flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            ...
            {disease && !loadingDb && (
              <p className="text-sm text-slate-400 mt-1">Analysing {disease.disease} with your patient profile</p>
            )}
          </div>
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-3 print:hidden">
            {[
              ...
              { label: 'Optional',    count: optional.length,    color: 'text-slate-600 bg-slate-50 border-slate-100' },
          ...
          {/* Actions */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow p-5 flex items-center justify-between gap-4 print:hidden">
            <div>
              <p className="text-sm font-medium text-slate-700">Next step</p>
              <p className="text-xs text-slate-400">Upload your blood test report for AI analysis</p>
            </div>
            <button
      ...
      {!loading && !loadingDb && fetched && tests.length === 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow p-10 text-center">
          ...
          <p className="text-slate-500">No test recommendations returned. Please try again.</p>
          <button
